...
def display_explanation(input_df, session, aws_bucket):
    ...
    st.pyplot(fig)
    # top feature
    top_feature = pd.Series(shap_values[0, :, 0].values, index=shap_values[0, :, 0].features_names).abs().idmax()
...
if submitted:
    ...
    if status == 200:
        ...
        display_explanation(input_df_transformed, session, aws_bucket)
    else:
        st.error(res)
